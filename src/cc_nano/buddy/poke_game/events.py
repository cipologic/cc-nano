...
def auto_event(session: GameSession, log_fn: Callable[[str], None]) -> None:
    ...
    positive = random.random() < 0.6  # 60% 概率为正面结果
    ...
    if event_id == "hurt":
        ...
        if positive:
            ...
            log_fn("   找到了一些野果，吃得心满意足！(SPD+1)")
        else:
            session.mood = max(0, session.mood - 5)
            log_fn("   到处找不到食物，只能饿着肚子继续走... (心情-5)")
        ...
        if positive:
            ...
            log_fn("   宝箱是空的...或许早就被人取走了。")

    elif event_id == "noise":
        log_fn(f"👂 {name} 竖起耳朵——远处传来了奇怪的声音...")
        if positive:
            state.add_tickets(2)
            log_fn("   勇敢地调查了声源，发现了一个藏宝处！(+2 奖券)")
        else:
            state.apply_stat_change("HP", -5)
            state.apply_stat_change("ATK", 1)
            log_fn("   遇到了一只小怪物，经过搏斗获得了经验！(HP-5, ATK+1)")
        ...
        if positive:
            ...
            log_fn("   仔细一看是一根普通的木棍... 但心意可嘉！")

    elif event_id == "npc_help":
        npc_species = random.choice(
            ["精灵", "矮人", "旅行商人", "流浪骑士", "神秘老者"]
        )
