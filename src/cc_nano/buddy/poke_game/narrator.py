"""LLM叙述者 — 基于gpt-5-mini的叙事驱动，附带回退模板。

使用专用的OpenAI LLM客户端。当LLM不可用时，回退到预写的叙事模板，
以便游戏可以离线运行。
"""
...
from cc_nano.buddy.poke_game.types import (NPC, GameSession, Location)
...
_FALLBACK_SKILLS: list[dict[str, Any]] = [
    ...
    {"name": "暗影步", "power": 25, "element": "shadow", "description": "在暗影中穿行"},
    {
        ...
        "power": 20,
        "element": "water",
        "description": "流水般的治愈之力",
    },
    {
        "name": "雷霆一击",
        "power": 40,
        "element": "wind",
        "description": "凝聚雷电的强力一击",
    },
...
class Narrator:
    ...
    def _maybe_summarize(self, session: GameSession) -> None:
        ...
        if not self._llm_available or not self._client:
            # 离线模式：简单截断拼接
            texts = [
                m["content"][:100] for m in to_summarize if m["role"] == "assistant"
            ]
    ...
    def narrate_npc_dialogue(
        self, npc: NPC, player_msg: str, session: GameSession
    ) -> str:
        """生成NPC对话。"""
        prompt = (
    ...
    def _parse_response(self, text: str) -> tuple[str, list[dict]]:
        ...
        return narrative, events[:2]  # 最多2个事件
    ...
    def _fallback_events(self, loc: Location, session: GameSession) -> list[dict]:
        """基于地点权重生成随机事件。"""
