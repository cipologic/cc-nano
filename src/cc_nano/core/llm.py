...
except ImportError as exc:  # pragma: no cover - 通过测试桩进行测试
...
def get_context_window_for_model(model: str) -> int:
    ...
    if "[1m]" in lowered or "1m" in lowered.split("-"):
        return 1_000_000
    # 为 DeepSeek 模型添加 1M 上下文窗口的特殊处理
    if "deepseek" in lowered and ("1m" in lowered or "1000000" in lowered):
        return 1_000_000
    return MODEL_CONTEXT_WINDOW_DEFAULT


# 最大输出令牌的上限 —— 用于自动升级场景。
# 根据搜索结果，DeepSeek-V4-Flash 与 Pro 的最大输出长度均为 384K tokens[reference:3][reference:4]。
...
def default_companion_model(provider: str, model: str) -> str:
    ...
    if provider == _OPENAI_PROVIDER:
        return "deepseek-v4-flash"
    # Anthropic provider
    # deepseek-v4-flash 也是 Anthropic 兼容的
    # 未来可根据 model 参数选择更合适的模型
    return "deepseek-v4-flash"
...
def supports_reasoning_effort(provider: str, model: str) -> bool:
    ...
    if provider != _OPENAI_PROVIDER:
        return False
    lowered = model.lower()
    # DeepSeek V4 系列模型支持 reasoning_effort[reference:5]。
...
class LLMClient:
    def __init__(
        self,
        provider: str = _OPENAI_PROVIDER,  # 默认使用 OpenAI 提供商以支持 DeepSeek
        ...
        if self.provider == _ANTHROPIC_PROVIDER:
            if not api_key or not api_key.strip():
                raise ValueError(
                    ...
                    "  2. 在配置文件中设置 api_key（如 .cc-nano.toml）\n"
                    "  3. 使用 --api-key 命令行参数"
                )
        elif self.provider == _OPENAI_PROVIDER:
            if not api_key or not api_key.strip():
                raise ValueError(
                    "缺少 OpenAI/DeepSeek API Key。\n"
                    "请通过以下方式之一提供：\n"
        ...
        if self.provider == _OPENAI_PROVIDER:
            if OpenAI is None:
                message = "OpenAI 支持需要安装 `openai` 包。"
                if _OPENAI_IMPORT_ERROR is not None:
                    message += f" 导入失败：{_OPENAI_IMPORT_ERROR}"
    ...
    def is_retryable_error(self, exc: Exception) -> bool:
        if isinstance(
            exc, (httpx.RemoteProtocolError, httpx.ReadError, httpx.ConnectError)
        ):
            return True
        if self.provider == _OPENAI_PROVIDER:
...
def _normalize_openai_stop_reason(reason: str | None) -> str | None:
    """将 OpenAI 的 finish_reason 映射为 Anthropic 的 stop_reason 对应值。"""
    if reason is None:
        return None
    _MAPPING = {
        "stop": "end_turn",
...
def _normalize_openai_message(message: Any) -> list[dict[str, Any]]:
    ...
    for tool_call in _value(message, "tool_calls", []) or []:
        arguments = (
            _value(_value(tool_call, "function", {}) or {}, "arguments", "") or ""
        )
        parsed_args: Any = {}
        if arguments:
            try:
...
def _build_openai_request(
    *,
    ...
    # 强制禁用 DeepSeek 思考模式，避免 reasoning_content 回传问题
    if "deepseek" in model.lower():
        params.setdefault("extra_body", {}).update({"thinking": {"type": "disabled"}})
